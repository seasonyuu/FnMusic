...
### 歌单歌曲访问状态（2026-09-22 补齐）

歌曲列表条目的 `accessStatus` 原样保留到 Android `Track` 及本地序列化数据。
Web 判定：缺省或 `0` 可播放，任意非零数值不可播放；`1/3` 显示文件缺失，
`2/4` 显示无访问权限，其余值仅显示不可用。不与音乐文件夹的状态码混用。
Android 保留失效歌曲及移除入口，禁止播放和加入队列，整张歌单播放过滤失效项，
点击有效歌曲时按过滤后的顺序映射索引（保留重复曲目）。

证据等级：源码核实（复核此前下载的 Web 部署资源，未执行真实 NAS 写入）。
资源 `1bc04b5291c26a46d918139138b992d2-LE4mTIM4.js` 中 `w_` 判定非零状态，
`T_` 映射提示，`O_` 保留 `accessStatus`；SHA-256：
`a1596e6da2fd2e53b0de4529e91044ab633a37bcb8a6256f1f59d748fd5765ca`。
本地协议测试覆盖缺省、null、0、1–4、未知值及持久化往返；设备测试使用模拟曲目。
