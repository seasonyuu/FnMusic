...
class LiquidGlassMaterialTest {
    @Test fun clearThroughDefaultIncreasesBlurAndTint() {
        ...
        for (step in 0..50) {
            ...
            assertEquals(1f + step / 25f, material.blurScale, 0.00001f)
            assertTrue(material.surfaceAlpha >= previousAlpha)
            previousAlpha = material.surfaceAlpha
        }
        assertEquals(0.45f, resolveLiquidGlassMaterial(0.1f).surfaceAlpha, 0f)
    ...
    @Test fun tintedHalfIncreasesBothBlurAndOpacityContinuously() {
        ...
        assertEquals(4.5f, middle.blurScale, 0f)
        assertEquals(0.65f, middle.surfaceAlpha, 0.00001f)
        val end = resolveLiquidGlassMaterial(2f)
        assertEquals(6f, end.blurScale, 0f)
        assertEquals(0.75f, end.surfaceAlpha, 0.00001f)
    ...
    @Test fun existingPreferencesAndInvalidValuesResolveSafely() {
        assertEquals(1f, resolveLiquidGlassMaterial(LiquidGlassBlur.Minimum).blurScale, 0f)
    ...
    @Test fun disabledGlassUsesFixedTintWithoutEffectsRegardlessOfIntensity() {
        for (value in listOf(0.1f, 1f, 2f, Float.NaN)) {
            val disabled = resolveLiquidGlassMaterial(value, enabled = false)
            assertEquals(false, disabled.enabled)
            assertEquals(0.60f, disabled.surfaceAlpha, 0f)
