...
FnMusic automatically tries a conservative title/artist/version match in [AMLL TTML DB](https://github.com/amll-dev/amll-ttml-db), then falls back to the fnOS lyrics endpoint when there is no unique match or the selected service is unavailable. The track menu's **选择歌词** action lets you search and preview alternatives, pin an AMLL selection, return to automatic matching, or always use fnOS lyrics for that track. AMLL supports word timing, translation, and a per-track timing offset; phonetic, background-vocal, and duet layouts are not included yet.

**Settings → 歌词 → AMLL 设置** provides a persistent AMLL enable switch and selects Bikonoo (default), GitHub, Dimeta, or GDBA. Disabling AMLL uses fnOS lyrics and disables the source, index-update, and cache-clear controls while preserving indexes, cached lyrics, track choices, and offsets. Only the selected service is queried; there is no automatic mirror fallback. Switching services always checks that service's index. The page displays the index's content-hash version, on-disk JSONL size, last check, and a manual update action. “Latest” refers to the selected service, which may lag upstream.

Lyrics are cached separately from audio and search indexes. Cache management shows the total document count and space used across services and can clear lyric files. Clearing keeps search indexes, track choices, offsets, and the lyrics currently displayed. Track choices are isolated by connection endpoint, account, and track GUID. Indexes and public lyric files are stored per service on the device. First use downloads the selected service's index; track metadata searches run locally.

AMLL's README distinguishes contributor-authored CC0 material from externally sourced material governed by its original terms. FnMusic displays lyric contributors in the lyric picker and links to the upstream repository from the picker and AMLL settings; it does not bundle the database.
